jQuery.fn.DrawCanvas = function (myJsonVariable) /* call : $('#test').DrawCanvas(); */
{
	...
	return this.each (function () 
	{
		...
		{
			time = myJsonVariable.TimeElasped; 
			timeGap = myJsonVariable.TimeGap != ""?myJsonVariable.TimeGap : 5;
			maxWidth = c.width - 10;
			if(time < 90)
			{
			...
			{
				pixelPerMinute = maxWidth/((90 + timeGap) + (Math.floor((time-90)/timeGap)*timeGap));
				pixelLoop = Math.floor(time/timeGap)+1;
			}
		...
		function drawTimeLine(time)
		{
			...
			//ctx.clearRect(0,10,timeElapsed-35,50);
			ctx.fillStyle = "rgba(255, 255, 255, 0.25)";//"yellow";
			ctx.fillRect(timeElapsed,10,630,80); // botx,topx,boty,topy
			ctx.fill();
		}
		...
		function drawField()
		{
			...
			for(var i=0;i<=pixelLoop;i++)
			{
				...
				ctx.fillRect(botx,30,botx+pixelPerTimeGap,40); // botx,topx,boty,topy // (0,70,638,40
				ctx.fill();
				drawLine(botx,boty-30);
				...
				if(i>0)
				 ...
				 ctx.font = "12px Helvetica, sans-serif";
					...
					ctx.fillStyle = "white";
					ctx.strokeText(text===0?timeGap:text, botx-9, 65);
					if(text===0)
		...
		{
			ctx.beginPath();
			ctx.moveTo(x,30);//ctx.moveTo(349,0);
		...
		function drawCard(Card,color,team)
		{
			...
			if(Card.length > 0)
			{
				...
				for(var i=0; i< Card.length; i++)
				{
					...
					if(team === true)
					{
						ctx.fillRect(timeElapsed-4,15,8,15); // botx,topx,boty,topy
					...
					{
						ctx.fillRect(timeElapsed-4,70,8,15);
						PointCollection.push({
							message: Card[i][1],
							width: 8,
							height: 15,
				...
				for(var i=0; i< Goal.length; i++)
				{
					...
					if( team === true)
					{
					   // ctx.arc(timeElapsed+10, 25, 5, 0, 2 * Math.PI, false);
						...
						PointCollection.push({
							message: Goal[i][1],//'Left foot shot from outside the D circle',
							width: 10,
							height: 10,
							x: timeElapsed-8, 
							y: 15,   
					...
					{
						//ctx.arc(timeElapsed+10, 75, 5, 0, 2 * Math.PI, false);
		...
		function drawReplacement(Replacement,team)
		{
			if(Replacement.length > 0)
			{
				...
				for(var i=0; i< Replacement.length; i++)
				{
					...
					{
					   // ctx.arc(timeElapsed+10, 25, 5, 0, 2 * Math.PI, false);
						ctx.drawImage(arrowImg, timeElapsed-4, 15,15,15);
						PointCollection.push({
							message: Replacement[i][1],
							width: 15,
							height: 15,
							x: timeElapsed-4, 
							y: 15,   
					...
					{
						//ctx.arc(timeElapsed+10, 75, 5, 0, 2 * Math.PI, false);
						ctx.drawImage(arrowImg, timeElapsed-4, 70,15,15);
						PointCollection.push({
							message: Replacement[i][1],
							width: 15,
							height: 15,
